...
class Analyze_graphics:
    ...
    def infer_and_draw(self, image_path, save_results=True):
        ...
        for detection in filtered_detections:
            ...
            cv2.imwrite(rf"{ready_path}", cropped_img)
            ...
            # 儲存二值化圖
            ready_binary_path = os.path.join(
                ready_dir, f"{image_name}_{index}_binary.jpg"
            )
            cv2.imwrite(rf"{ready_binary_path}", binary_inverted)
