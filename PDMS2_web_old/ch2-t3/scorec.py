...
class CrossScorer:
    ...
    @staticmethod
    def keep_largest_component(skel, clear_border=True):
        S = (skel>0).astype(np.uint8)
        num,lab,stats,_ = cv2.connectedComponentsWithStats(S,8)
        if num<=1: 
            return S*255
        idxs = list(range(1,num))
        if clear_border:
            H,W = S.shape
            idxs = [i for i in idxs if not (stats[i,0]==0 or stats[i,1]==0 or stats[i,0]+stats[i,2]>=W-1 or stats[i,1]+stats[i,3]>=H-1)] or list(range(1,num))
        best = max(idxs, key=lambda i: stats[i,cv2.CC_STAT_AREA])
        out = np.zeros_like(S); out[lab==best]=255
        return out
    ...
    def robust_cross_from_skeleton(self, skeleton):
        S = self.keep_largest_component(skeleton, True)
    ...
    def skeletonize(binary):
        try:
            ...
            img=binary.copy(); skel=np.zeros_like(img); k=cv2.getStructuringElement(cv2.MORPH_CROSS,(3,3))
            while True:
                eroded=cv2.erode(img,k); opened=cv2.dilate(eroded,k); temp=cv2.subtract(img,opened)
    ...
    def score_image(self, img_path):
        ...
        ok,reason,P,p1,p2,theta,ends=self.robust_cross_from_skeleton(skeleton)

        vis=img.copy()
        skel_vis=cv2.dilate(skeleton,cv2.getStructuringElement(cv2.MORPH_RECT,(3,3)),1)
        vis=cv2.addWeighted(vis,1.0,cv2.cvtColor(skel_vis,cv2.COLOR_GRAY2BGR),0.5,0)
...
if __name__ == "__main__":
    ...
    results = scorer.score_folder("image", "img30.jpg")#"img*.jpg"
    print("所有結果:", results)
