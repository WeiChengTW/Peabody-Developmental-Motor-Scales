...
class Analyze_graphics:
    ...
    def infer_and_draw(self, image_path, save_results=True, thresh_path=None):
        ...
        input_img = None
        if mode == "black":
            input_img = cv2.imread(thresh_path)
        elif mode == "org":
            input_img = cv2.imread(ori_path)
        ...
        for detection in filtered_detections:
            ...
            # 信心度<0.5的歸類為Other
            if confidence < 0.5 or self.class_names[cls_id] != "Cross":
            ...
            if save_results:
                ...
                x1, y1, x2, y2 = (
                    max(0, int(x1)),
                    max(0, int(y1)),
                    min(input_img.shape[1], int(x2)),
                    min(input_img.shape[0], int(y2)),
                )
                ...
                if x2 > x1 and y2 > y1:  # 確保有效的裁切區域
                    ...
                    filename = f"ch2-t3\\{image_name}_{index}.jpg"
        ...
        if save_results:
            ...
            annotated_image_path = os.path.join(
                result_dir, f"ch2-t3\\{image_name}_annotated.jpg"
            )
