...
def analyze_paint(image, y_top, y_bot, show_windows=False):
    ...
    lower1 = np.array([0, 35, 35],  np.uint8); upper1 = np.array([10, 255, 255], np.uint8)
    lower2 = np.array([170,35, 35], np.uint8); upper2 = np.array([180,255, 255], np.uint8)
    ...
    h, w = mask_red.shape
    margin = 20
    mask_not_near_edge = (xs > margin) & (xs < w - margin) & (ys > margin) & (ys < h - margin)

    tol = 2
    over_mask = ((ys < (y_top - tol)) | (ys > (y_bot + tol))) & mask_in_xrange & mask_not_near_edge
    over_ys = ys[over_mask]; over_xs = xs[over_mask]
