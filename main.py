...
def analyze_image(img_path, cm_per_pixel=0.02068, actual_length_cm=7.5):
...
if __name__ == "__main__":
    ...
    img_id = sys.argv[2] if len(sys.argv) > 2 else "unknown"

    # 可透過後端傳值或用原本預設的
    cm_per_pixel     = _safe_float(sys.argv[3], 0.02068) if len(sys.argv) > 3 else 0.02068
    actual_length_cm = _safe_float(sys.argv[4], 7.5)     if len(sys.argv) > 4 else 7.5

    
    image_path = rf"kid\{uid}\{img_id}.jpg"

    score = analyze_image(image_path, cm_per_pixel, actual_length_cm)
    print(f"score : {score}")
    return_score(score)