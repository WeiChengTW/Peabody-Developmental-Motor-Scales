...
class Analyze_graphics:
    def __init__(
        self,
        model_path=r"C:\Users\chang\Downloads\circle\runs\segment\train\weights\best.pt",
    ...
    def infer_and_draw(self, image_path, save_results=True):
        ...
        original_image = image.copy()  # 保存原始圖像
        ...
        for detection in filtered_detections:
            ...
            if save_results:
                ...
                x1, y1, x2, y2 = (
                    max(0, int(x1)),
                    max(0, int(y1)),
                    min(original_image.shape[1], int(x2)),
                    min(original_image.shape[0], int(y2)),
                )
