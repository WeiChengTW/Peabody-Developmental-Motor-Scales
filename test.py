...
def insert_score(uid: str, task_id: str, score: int, no: int | None = None, test_date: date | None = None) -> str:
    """
    ...
    try:
        ...
        write_to_console(f"[DB] insert_score ok: uid={uid}, task_id={task_id}, score={score}, score_id={score_id}", "INFO")
        return score_id
    except Exception as e:
        write_to_console(f"[DB] insert_score failed: uid={uid}, task_id={task_id}, err={e}", "ERROR")
        raise
...
def run_analysis_in_background(task_id, uid, img_id, script_path):
    try:
        ...
        cmd = [sys.executable, str(script_path), uid, img_id] if uid else [sys.executable, str(script_path), img_id]
...
def run_python_script():
    try:
        ...
        t = Thread(target=run_analysis_in_background, args=(task_id, uid, img_id, script_path))
        t.daemon = True
        t.start()
...
def capture_opencv_photo():
    ...
    try:
        ...
        t = Thread(target=run_analysis_in_background,
