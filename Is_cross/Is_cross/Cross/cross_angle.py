...
def is_cross_angle_hough_select(img_path, threshold=15, debug=True, scale=3):
    ...
    lines = cv2.HoughLinesP(
        ...
        np.pi / 180,
        threshold=50,
        minLineLength=0.3 * min(binary.shape),
        maxLineGap=20,
    )
    ...
    is_not_rightangle = angle_diff > threshold

    if debug:
        print(
            f"{os.path.basename(img_path)}: 主橫線+主直線交角={cross_angle:.1f}° (偏差={angle_diff:.1f}°)"
        )
...
#     is_cross_angle_hough_select(img_path, threshold=15, debug=True, scale=3)
img_path = "test/83.jpg"
is_cross_angle_hough_select(img_path, threshold=15, debug=True, scale=3)
