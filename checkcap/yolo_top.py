...
# cap = cv2.VideoCapture(r"C:\Users\hiimd\Downloads\Demo\瓢蟲正.mp4")
...
while True:
    ...
    if ladybug_box:
        ...
        # 只有當偵測到剛好6個洞才更新（避免異常誤偵測）
        if len(temp_holes) == 6:
            temp_holes.sort(key=lambda h: (h["pos"][1], h["pos"][0]))
            hole_positions = temp_holes.copy()
    ...
    # cv2.putText(frame, f"cap_ng: {cap_ng_count}", (10, 50),
