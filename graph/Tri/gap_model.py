...
def build_model(PX=100):
    model = models.Sequential([
        ...
        layers.Flatten(),
        layers.Dense(1024, activation='relu', kernel_regularizer=regularizers.l2(3e-2)),
        layers.Dropout(0.2),
...
def main(save):
    ...
    model.compile(
        ...
        loss=binary_focal_loss(gamma=1.3955, alpha=0.5555),
        metrics=['accuracy']
    )

    lr_scheduler = ReduceLROnPlateau(monitor='val_loss', factor=0.55, patience=3, min_lr=5e-5)
    ...
    acc, f1_score = extract_macro_f1(y_test_single, y_pred_bin)
    print(f"acc : {acc}, f1_score : {f1_score}")
