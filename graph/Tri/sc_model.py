...
def build_model(PX=100):
    model = models.Sequential([
        ...
        layers.Flatten(),
        layers.Dense(512, activation='relu', kernel_regularizer=regularizers.l2(4e-2)),
        layers.Dropout(0.45),
...
def main(save):
    ...
    model.compile(
        ...
        loss=binary_focal_loss(gamma=0.9023, alpha=0.7555),
        metrics=['accuracy']
    )

    lr_scheduler = ReduceLROnPlateau(monitor='val_loss', factor=0.5555, patience=3, min_lr=5e-5)
