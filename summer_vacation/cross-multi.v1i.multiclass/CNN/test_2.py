...
# 載入模型時不編譯，避免自定義函數問題
models = {
    "Asymmetry": load_model("../models/Asymmetry_best_model.keras", compile=False),
    "Unlabeled": load_model("../models/Unlabeled_best_model.keras", compile=False),
    "no line": load_model("../models/no line_best_model.keras", compile=False),
    "non right angle": load_model(
        "../models/non right angle_best_model.keras", compile=False
    ),
    "other": load_model("../models/other_best_model.keras", compile=False),
}

test_folder = "train"  # 測試資料夾名稱
...
# 用於收集所有預測和真實標籤的字典
all_predictions = {feature: [] for feature in label_columns}
all_true_labels = {feature: [] for feature in label_columns}
all_confidences = {feature: [] for feature in label_columns}
...
for i, row in enumerate(valid_rows):
    ...
    # 對每個特徵顯示結果
    for j, feature in enumerate(label_columns):
