...
def main():
    ...
    model = models.Sequential(
        [
            layers.Conv2D(32, (3, 3), activation="relu", input_shape=(PX, PX, 3)),
            ...
            layers.Flatten(),
            layers.Dense(
                512, activation="relu", kernel_regularizer=regularizers.l2(2e-2)
            ),
            layers.Dropout(0.2),
            layers.Dense(1, activation="sigmoid"),  # 只有一個輸出節點for gap
        ]
    )

    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=2e-3),
        loss=binary_focal_loss(gamma=2.55, alpha=0.555),
        metrics=["accuracy"],
    )
    ...
    if y_test.ndim > 1 and y_test.shape[1] > 1:
        # 只取 gap 列 (第一列)
        y_test_gap = y_test[:, 0:1]
    else:
        y_test_gap = y_test

    # CM.plot_confusion_matrices(y_test_gap, y_pred_binary, label_columns)
