...
class ShapeAnalyzer:
    ...
    def process(self, debug=False):
        ...
        adaptive_thresh = cv2.adaptiveThreshold(
            blurred, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 13, 2
        )
        ...
        thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel_final, iterations=2)
        ...
        img_prefix = base_name.split("_extracted_paper")[0]
        thresh_path = f"test_img/{img_prefix}_thresh.jpg"
...
if __name__ == "__main__":
    img = "img5"
    detector = ShapeAnalyzer(image_path=f"test_img\{img}_extracted_paper.jpg")
