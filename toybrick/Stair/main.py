...
def analyze_image_side(img_path, initial_score, ori_type, model):
    ...
    if MODE == 0:  # 階梯模式
        ...
        grouper = LayerGrouping(layer_ratio=0.2)
        layers = grouper.group_by_y(centroids, boxes=boxes)
        block_width = np.mean([box[2] - box[0] for box in boxes]) // 2 if len(boxes) > 0 else 0
...
def analyze_image_top(frame, initial_score, model):
    ...
    if status_offset == 'Offset !' or status_rotate == 'Rotate !' or status_rotate == 'No Bricks':
        GET_POINT = 1
        color = (0, 0, 255)
    else:
        color = (0, 0, 0)
...
if __name__ == "__main__":
    ...
    # --- 圖片路徑設定 ---
    # 請修改為您的實際檔案路徑
    SIDE_IMG_PATH = r"left_stair.jpg" 
    ...
    try:
        ...
        summary, score_top, analyzed_frame_top = analyze_image_top(frame_top, INITIAL_SCORE, yolo_model)
