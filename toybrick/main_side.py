...
#                     cy = int(np.mean(ys))
#                     label = f"{names[classes[i]]} {confs[i]:.2f}"
#                     cv2.putText(annotated, label, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
#         combined = cv2.addWeighted(annotated, 0.8, frame, 0.2, 0)
#         combined = cv2.resize(combined, (0, 0), fx=2, fy=2)
...
#                 bbox_widths = [box[2] - box[0] for box in results[0].boxes.xyxy.cpu().numpy()]
#                 avg_width = np.mean(bbox_widths) if bbox_widths else 1
#                 # GAP_THRESHOLD_RATIO = cv2.getTrackbarPos("Pyramid X Threshold", "YOLOv8-segmentation with Gap Detection") // 100
...
#                         mid_x = int((p1[0] + p2[0]) / 2)
#                         mid_y = int((p1[1] + p2[1]) / 2)
#                         cv2.putText(frame, f"{d:.1f}", (mid_x, mid_y + 20),
...
#         block_width = np.mean([box[2] - box[0] for box in results[0].boxes.xyxy.cpu().numpy()]) // 1.222
#         grouper = LayerGrouping()
#         layers = grouper.group_by_y(centroids)
...
#         annotated = frame.copy()
        ...
        #             label = f"{names[classes[i]]} {confs[i]:.2f}"
        #             cv2.circle(annotated, (cx, cy), 3, (0, 0, 0), -1)
        #             cv2.putText(annotated, label, (cx, cy + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (111, 111, 111), 2)
